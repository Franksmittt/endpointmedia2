    ...
    <section className="py-20 bg-gradient-to-b from-black via-gray-950 to-black text-white relative overflow-hidden" aria-labelledby="social-proof-title">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="max-w-3xl mb-12">
          ...
          <h2 id="social-proof-title" className="text-3xl md:text-4xl lg:text-5xl font-bold text-white mt-3 bg-gradient-to-r from-white via-cyan-100 to-white bg-clip-text text-transparent">
          ...
          {testimonials.map((testimonial) => (
            ...
            >
              ...
              <div className="relative z-10">
                <p className="text-zinc-400 italic text-base md:text-lg mb-6">&ldquo;{testimonial.quote}&rdquo;</p>
                <div className="mt-6">
                  <p className="font-semibold text-white">{testimonial.author}</p>
                  <p className="text-sm text-zinc-500">{testimonial.role}</p>
                  <Link
                    href={testimonial.link}
                    className="text-cyan-400 text-sm font-semibold inline-flex items-center mt-2 hover:text-teal-400/90 transition-colors"
                  >
                    See how &rarr;
                  </Link>
                </div>
        ...
        <div className="grid gap-6 md:grid-cols-3 mt-12">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="group relative bg-gradient-to-br from-gray-900/90 via-black/80 to-gray-900/90 backdrop-blur-sm border border-teal-400/30 rounded-2xl p-6 text-center shadow-2xl shadow-cyan-500/10 hover:shadow-cyan-400/20 hover:border-cyan-400/50 transition-all duration-300"
            >
              {/* Glowing Top Border */}
              <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-cyan-400/50 to-transparent opacity-50 group-hover:opacity-100 transition-opacity"></div>
